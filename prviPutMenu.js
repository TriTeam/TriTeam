...
const stavkeTutoriala=[
  1,
  {text:'HI, here is quick tutorial how to use this game...(tap to highlighted tabs to continue...)',id:'',next:'crnaMagla',pad:'40dvh',tabla:''},
  {text:'Here you can add your friends to your friend list by entering their ID',id:'dodaj_prijatelja',next:'prijateljiBack',pad:'40dvh',tabla:'prijateljiTabla'},
  {text:'Here you can find all your data such as: name, races won, races finished, best race time, your rank and ranking info, your ID',id:'info_igrac',next:'info_igrac',pad:'150dvh',tabla:''},
  {text:'Here you can find all the rules and scoring sistem',id:'rules',next:'pravilaBack',pad:'120dvh',tabla:'pravilaDiv'},
  {text:'Here you can join the game with game ID after your friend gives you one',id:'udi_igru',next:'udiIgruBack',pad:'120dvh',tabla:'udiIgru'},
  {text:'Here you can create the game and then invite your friends',id:'kreiraj_igru',next:'kreiraj_igru',pad:'110dvh',tabla:''}
]
let mojid;
brisiKajNetreba()

async function brisiKajNetreba() {
  const put=ref(db,'IGRA/IGRE')
  const snapshot=await get(put);
  if(snapshot.exists()){
    for(let i=0; i<Object.keys(snapshot.val()).length;i++){
      const pu2=ref(db,'IGRA/IGRE/'+Object.keys(snapshot.val())[i]+'/id');
      const snapshot2=await get(pu2);
      if(!(snapshot2.exists())){
        const put3=ref(db,'IGRA/IGRE/'+Object.keys(snapshot.val())[i]);
        remove(put3)
      }
...
async function provjeraPrviPut(id) {
  ...
  if(snapshot.exists()) {
    ...
    const menuUlogiranje=snapshot.val().menu+1;
    const novo={
      menu:menuUlogiranje,
      tutorial:false
    }
...
function tutorial(){
  ...
  document.getElementById(ovastavka.id).addEventListener("click", function () {
    magla.innerHTML=''
    document.getElementById(ovastavka.id).style.zIndex='';
}, { once: true });
  }
  
  document.getElementById(ovastavka.next).addEventListener("click", function () {
    stavkeTutoriala[0]++;
    if(stavkeTutoriala[0]!=7){
      tutorial()
    }else{document.getElementById('tutorial').style.display='block'}
}, { once: true });
