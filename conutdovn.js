...
async function sve(id,ime,igra){
  ...
  for(let i=0;i<sortedPlayers.length;i++){
    ...
    if(i==0){
      ...
      document.getElementById('player').style.backgroundColor=igra.postavkeIgraca[id].boja;
    
    }else{
      //tu ce ici igraci u tu stazu
      console.log()
      const bojaIgraca=igra.postavkeIgraca[sortedPlayers[i]].boja;
      const putDoRanka=ref(db,'IGRA/KORISNICI/'+sortedPlayers[i]+'/rank');
  ...
  if(mP=='p'){
    if(gascina<3){
      if(!(gascina==maxgas)){
        gascina++;
        console.log(gascina)
        document.getElementById('jacina'+(gascina+2)).style.backgroundColor='red'
    ...
    if(gascina>-1){
      gascina--;
      console.log(gascina)
      document.getElementById('jacina'+(gascina+3)).style.backgroundColor='white'
    }
  }gas()
}
